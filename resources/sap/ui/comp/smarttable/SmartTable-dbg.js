...
sap.ui.define([
    ...
    SmartTable.prototype._validateTextInEditModeSource = function(oConfig) {
		var aAllowedValues = Object.keys(library.smartfield.TextInEditModeSource);
        if (typeof oConfig["textInEditModeSource"] !== "object"
            || !Object.keys(oConfig["textInEditModeSource"]).length
			|| oConfig["textInEditModeSource"].isA) {
        ...
        for (var sKey in oConfig["textInEditModeSource"]) {
            if (!aAllowedValues.includes(oConfig["textInEditModeSource"][sKey])) {
                Log.error("Invalid config property '" + sKey + "' for the textInEditModeSource property of the customizeConfig - " + this.getId());
                delete oConfig["textInEditModeSource"][sKey];
            }
        }
    };
	...
	SmartTable.prototype._openExportSettings = function() {
		...
		this._loadExportLibrary().then(function() {
			sap.ui.require(['sap/ui/export/ExportUtils'], function(ExportUtils) {
				...
				ExportUtils.getExportSettingsViaDialog(that._cachedExcelSettings, that, undefined, bEnablePDFExport
	...
	SmartTable.prototype._createTableProvider = function() {
		var oModel, sEntitySetName, sIgnoredFields, bPreserveDecimals, oCustomConfig;
		sEntitySetName = this.getEntitySet();
		sIgnoredFields = this.getIgnoredFields();
		oModel = this.getModel();
		bPreserveDecimals = this.data("preserveDecimals");
		oCustomConfig = this.getCustomizeConfig();
		this._oTextInEditModeSource = oCustomConfig && oCustomConfig["textInEditModeSource"];
		...
		if (oModel && sEntitySetName) {
			...
			this._oTableProvider = new TableProvider({
				...
				textInEditModeSource: this._oTextInEditModeSource
			});
		}
	};
	...
	SmartTable.prototype._createAnalyticalColumn = function(oField, sId) {
		...
		var oColumn = new AnalyticalColumn(sId, {
			...
			template: oField.template,
			groupHeaderFormatter: FormatUtil.getInlineGroupFormatterFunction(oField, true, this._getDateFormatSettings(), this._bPreserveDecimals, true /* bReplaceWhitespace */)
		});
		...
		if (this._isMobileTable && this._oCurrentVariant.group && this._oCurrentVariant.group.groupItems) {
			...
			if (sPath) {
				...
				if (oFieldMetadata) {
					...
					fnGroupFunction = FormatUtil.getInlineGroupFormatterFunction(oFieldMetadata, false, this._getDateFormatSettings(), this._bPreserveDecimals, true /* bReplaceWhitespace */);
				}
