...
sap.ui.define([
	...
	RedlineToolHandler.prototype.beginGesture = function(event) {
		...
		if (gizmo && this._inside(event)) {
			...
			gizmo._activeElement = gizmo.getAggregation("activeElement");
			if (gizmo._activeElement) {
				...
				this._pivotPoint = gizmo._toVirtualSpace(event.x - offset.x, event.y - offset.y);
			}
		}
	};
	...
	RedlineToolHandler.prototype.zoom = function(zoomDelta) {
		var gizmo = this._tool.getGizmo();
