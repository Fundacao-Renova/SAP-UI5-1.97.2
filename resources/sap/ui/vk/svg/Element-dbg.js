...
sap.ui.define([
	...
	Element.prototype._setBaseAttributes = function(setAttributeFunc, mask) {
		...
		if (this.opacity !== undefined) {
			// for a hotspot, only if it's selected and showing (having tint colour ), the opacity is re-calculated
			setAttributeFunc("opacity", this.nodeContentType === NodeContentType.Hotspot && this.isSelected(mask) && this.tintColor ? (this.opacity * 0.5 + 0.5) : this.opacity);
		}
