			...
			nodes.forEach(function(node) {
				if (node.opacity !== undefined && node.domRef) {
					if (value) {
						...
						node.resetOpacity();
						if (vsm.getSelectionState(node)) {
							selectedHotspotNodes.push(node);
						}
					}
				}
			});

			// for no-showing and selected hotspots, reset selection,
			// so hotspots go back to original selection status
			if (selectedHotspotNodes.length) {
				vsm.setSelectionState(selectedHotspotNodes, false);
				vsm.setSelectionState(selectedHotspotNodes, true);
			}
			if (nodes.length > 0) {
				this.invalidate();
			}
	...
	Viewport.prototype.showHotspots = function(nodeRefs, show, color) {
		...
		nodeRefs.forEach(function(node) {
			if (show) {
				...
				node.resetOpacity();
				if (vsm && vsm.getSelectionState(node)) {
					selectedHotspotNodes.push(node);
				}
			}
		}, this);
		...
		// for no-showing and selected hotspots, reset selection,
		// so hotspots go back to original selection status
		if (vsm && selectedHotspotNodes.length) {
			vsm.setSelectionState(selectedHotspotNodes, false);
			vsm.setSelectionState(selectedHotspotNodes, true);
		}
