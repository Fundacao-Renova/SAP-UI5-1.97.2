	...
	var oMethods = {
		getMethods: function (oViewProxy, oTemplateUtils, oController) {
			...
			function addEntryWithFilters(oEvent) {
				var oTemplatePrivateModel = oTemplateUtils.oComponentUtils.getTemplatePrivateModel();
				var bCreateWithFiltersButtonEnabled = oTemplatePrivateModel.getProperty("/generic/bDataAreShownInTable");
				// Shortcut Key: Don't trigger the shortcut if the create with filters button is disabled
				if (bCreateWithFiltersButtonEnabled) {
