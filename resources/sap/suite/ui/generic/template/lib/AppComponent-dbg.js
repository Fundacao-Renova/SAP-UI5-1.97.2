	...
	function getMethods(oAppComponent, oAppId) {
		...
		function setGlobalTemplateModels(){
			...
			oCurrentUrlPromise.then(function(sURL) {
				...
				var oIsIntentUrlPromise = oParsingSerive.isIntentUrlAsync(sURL);
				oIsIntentUrlPromise.then(function(isIntentUrlResponse){
					oTemplateContract.oTemplatePrivateGlobalModel.setProperty("/generic/crossAppNavSupport", isIntentUrlResponse);
				}); 
				return oIsIntentUrlPromise;
			}); 
			var oMessageModel = sap.ui.getCore().getMessageManager().getMessageModel();
