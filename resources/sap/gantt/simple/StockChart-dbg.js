...
sap.ui.define([
	...
	"./UtilizationChart",
	"sap/ui/core/Core"
], function(merge, UtilizationChart, Core){
	"use strict";
				...
				if (aDimensionPeriods[i - 1] && aDimensionPeriods[i - 1].getTo().getTime() == aDimensionPeriods[i].getFrom().getTime()){
					...
					if (oNearestThresholdPeriod) {
						...
						oNearestThresholdPeriod.getTo().getTime() != ocurrentThreshold.getFrom().getTime()){
							...
							aCombinedPeriod.push(newPeriod);
						}
					}
				}
	...
	*/
	 ...
	 * @private
	*/
	StockChart.prototype._getFilterThresholdPeriods = function(aThresholdPeriods, oDimensionPeriod) {
	...
	*/
	StockChart.prototype._createIntersectionThresholdPaths = function(aDimensionPeriods, aThresholdPeriods, oDimensionPeriod, oDimension, aCombinedPeriod, i,  isPositiveThreshold) {
		...
		if (oStartDate.getTime() != oEndDate.getTime()) {
			...
			var aFilteredThreshold = this._getFilterThresholdPeriods(aThresholdPeriods, oDimensionPeriod);
	...
	StockChart.prototype._renderDimensionPaths = function(oRm, mProp) {
		...
		if (oMainDimension) {
			...
			if ( aNegtivePeriods.length > 0 && aPositivePeriods.length > 0) {
				...
				var aThresholdperiod = aPositivePeriods.length > 0 ? aPositiveThresholds : aNegtivePeriods;
	...
	*/
	StockChart.prototype.renderDimensionPath = function(oRm, oDimension, iRowY, iRowHeight, sIdSuffix, aDimensions, bIsDefsCreation) {
		if (oDimension.getIsThreshold() && bIsDefsCreation) {
			...
			var aDimensionPeriods = this._createThresholdDefsPath(oDimension, oMainDimension[0]);
	   }
		...
		for (var i = 0; i < aPeriods.length; i++) {
			...
			var xPos1 = this.toX(oDimension.getIsThreshold() && bIsDefsCreation ? oPeriod.from : oPeriod.getFrom());
			var xPos2 = this.toX(oDimension.getIsThreshold() && bIsDefsCreation ? oPeriod.to : oPeriod.getTo());
			var vValue = oDimension.getIsThreshold() && bIsDefsCreation ? oPeriod.value : oPeriod.getValue();
